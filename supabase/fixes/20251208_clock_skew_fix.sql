...
-- BM25 Clock Skew Fix
-- ============================================
-- Issue: Supabase server clock is 2 days behind local time
-- Cause: Test messages with JS timestamps appear "in the future" to Postgres
-- Fix: Increase the clock skew buffer from 5 seconds to 7 days
--
...
BEGIN
  ...
  SELECT
    ...
    ct.created_at::TIMESTAMPTZ, NULL::FLOAT AS vector_similarity,
    calculate_gravity_score(0.7, COALESCE(ct.impact_score, 0), COALESCE(ct.intimacy_level, 0),
      ct.created_at::TIMESTAMPTZ, COALESCE(ct.last_accessed, ct.created_at::TIMESTAMPTZ),
      COALESCE(ct.access_count, 0))::FLOAT AS gravity_score,
    ct.impact_score::INT, ct.intimacy_level::INT, ct.access_count, ct.last_accessed,
  ...
  ORDER BY ts_rank_cd(ct.content_tsvector, v_tsquery, 32) DESC
  LIMIT match_count;
END;
$$ LANGUAGE plpgsql STABLE;

SELECT 'BM25 clock skew fix deployed successfully' AS status;
