-- Migration: Add is_question column to chat_turns + filter in RPCs
-- Purpose: Extend Phase 1 question filtering from messages table to chat_turns table
-- The edge function path queries chat_turns, not messages, so it was unguarded.

-- 1. Add is_question column to chat_turns (default FALSE)
ALTER TABLE chat_turns ADD COLUMN IF NOT EXISTS is_question BOOLEAN DEFAULT FALSE;

-- 2. Partial index: only index non-question rows (same pattern as messages table)
CREATE INDEX IF NOT EXISTS idx_chat_turns_is_question
  ON chat_turns(is_question) WHERE is_question = FALSE;

-- 3. Update match_messages_with_gravity to exclude questions
...
-- 4. Update graph_walk_from_entities to exclude questions
...
-- 5. Update get_newest_turns_for_entities to exclude questions
