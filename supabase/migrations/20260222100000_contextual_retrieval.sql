-- Migration: Contextual Retrieval for KYT Memory
-- Purpose: Add LLM-generated context summaries to chat_turns chunks before embedding.
-- Asymmetric embedding: stored chunks get context prefix, query embeddings stay raw.
-- See: Anthropic Contextual Retrieval technique (49% retrieval failure reduction).

-- 1. Add columns to chat_turns
ALTER TABLE chat_turns ADD COLUMN IF NOT EXISTS contextual_content TEXT;
ALTER TABLE chat_turns ADD COLUMN IF NOT EXISTS context_generated BOOLEAN DEFAULT FALSE;
ALTER TABLE chat_turns ADD COLUMN IF NOT EXISTS context_generated_at TIMESTAMPTZ;

-- 2. Partial index for efficient backfill queries
-- Targets rows that haven't been contextualized yet.
CREATE INDEX IF NOT EXISTS idx_chat_turns_context_not_generated
ON chat_turns(created_at DESC)
WHERE context_generated = FALSE OR context_generated IS NULL;

-- 3. Update match_messages_with_gravity to return contextual_content
...
-- 4. Update graph_walk_from_entities to return contextual_content
...
-- 5. Update get_newest_turns_for_entities to return contextual_content
...
COMMENT ON FUNCTION get_newest_turns_for_entities IS
    ...
    'Excludes deflection turns (deflection >= 0.70). Includes contextual_content for BM25/reranking.';
