...
// ── Constants ─────────────────────────────────────────────────────────────
const HYDE_TIMEOUT_MS = 4000; // 4s budget — must stay within injection deadline

const HYDE_SYSTEM_PROMPT = `You are a memory recall assistant. Given a user's search query about a past conversation, generate a short hypothetical conversation snippet (2-3 turns, 100-200 words) that would be semantically similar to what the user is looking for.

The conversation should:
- Use natural, conversational language
- Include specific details that someone might remember
- Cover the topic from the perspective of a real conversation
